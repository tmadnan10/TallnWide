...
class PCAUtils {
	...
	*/
	...
	public static void outerProductWithIndices(org.apache.spark.mllib.linalg.Vector yi, Vector ym, double[] xi
			, double[][] resArray, int[] nonZeroIndices, int start, int end) {
		// 1. Sum(Yi' x (Xi-Xm))
	...
	public static Matrix updateXtXAndYtx(Matrix realCentralYtx,
		...
		//System.out.println(realCentralSumX.getQuick(0) - (nRows) * xm.getQuick(0));
...
//				if (col == 0) {
//					System.out.println(matrix.getQuick(index, col) +" x "+value);
//				}
			}
			resArray[col] = dotRes;
		}
	...
	*/
	...
	public static void printStatToFile(Stat stat, String outputPath) {
		try {
			...
			String memory="Driver Memory: "+stat.maxMemory;
			String partition="No of Partitions of W: "+stat.nPartitions;
			...
			String IO="Total IO Operation Time: "+stat.IOTime;
			...
			String dw="Change in W in each iteration: [";
			for(int i=0;i<stat.dwList.size()-1;i++){
				dw+=stat.dwList.get(i)+", ";
			}
			if(!stat.dwList.isEmpty())
				dw+=stat.dwList.get(stat.dwList.size()-1)+"]";
		...
		System.out.println("Written to stat "+outputPath);	
	}
